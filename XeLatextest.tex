...
\renewcommand{\kb}{3cm}
\jd{由题设~$\overrightarrow{AB}=(1,1,1),\overrightarrow{AC}=(2,1,3)$,  \dotfill{}(2')

  故~$\overrightarrow{AB}\times \overrightarrow{AC}=
  \begin{vmatrix}
   \bs{i}&\bs{j} &\bs{k}\\
   1&1&1\\
   2&1&3\\
...
\renewcommand{\kb}{3cm}			
\jd{$\dfrac{\partial z}{\partial x} = 2xf_1' + \dfrac{1}{y}f_2'$, \dotfill{}(3')
...
\renewcommand{\kb}{4cm}
\jd{~$\ds{\iint\limits_{D}\dfrac{\sin x}{x}\dif \sigma}
	=  \ds{\int_0^\pi\dfrac{\sin x}{x}\dif x\int_{0}^{\pi x}\dif y}$  \dotfill{}(2')
...
\jd{~$D: 0\leqslant \theta \leqslant 2\pi, 0\leqslant \rho \leqslant 5$  \dotfill{}(2')
...
\jd{~$\Omega = \{(x,y,z)\mid x^2 + y^2 \leqslant z^2,0\leqslant z\leqslant 4\}$ \dotfill{}(2')
...
求幂级数~$\ds{\sum\limits_{n=1}^\infty\dfrac{(x-1)^n}{n\cdot3^n}} $ 的收敛域与和函数~$s(x)$.
\jd{令~$t = x-1$,  上述级数变为
	 $\ds{\sum\limits_{n=1}^\infty\dfrac{(x-1)^n}{n\cdot3^n}} $  \dotfill{}(2')
